...
int main()
{
    ...
    Read_data_matrix();
    total_cost=prims();
    Write_minimum_spanning_tree_Prim(total_cost);
    return 0;
}
...
void update_distance_array( int visited_nodes[MAX], int cost_matrix[MAX][MAX], int distance[MAX], int from[MAX], int adiacent_node ) {
...
int prims()
{
    ...
    while(no_of_edges > 0)
    {
        ...
        second_adiacent_node = find_adiacent_node_minim_edge(visited, distance, &min_distance);
        ...
        update_distance_array(visited, cost, distance, from, second_adiacent_node);
